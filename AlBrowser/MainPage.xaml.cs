...
namespace Al_Browser
{
    ...
    public sealed partial class MainPage : Page
    {
        ...
        public MainPage()
        {
            ...
            ItemsTabs.Add(new Tab(0, "Yandex", "http://ya.ru"));
        ...
        private void AddressBar_QuerySubmitted(AutoSuggestBox sender, AutoSuggestBoxQuerySubmittedEventArgs args)
        {
            ...
            {
                WView.Source = new Uri("https://www.ya.ru/search?q=" + args.QueryText);
            }
            else
            {
                string url = args.QueryText.StartsWith("http") 
                    ? args.QueryText 
                    : "http://" + args.QueryText;
        ...
        private void AddressBar_TextChanged(AutoSuggestBox sender, 
        ...
        private void WView_LoadCompleted(object sender, NavigationEventArgs e)
        {
            ...
            while (history.Step() == SQLiteResult.ROW)
            {
                ...
                {
                    objConn.Prepare(
                        "UPDATE history SET times=" + (int.Parse(history[3].ToString()) + 1) 
                        + " WHERE url='" + WView.Source.ToString() + "';").Step();
                    return;
                }
            }

            objConn.Prepare(
                "INSERT INTO history (name, url, times) VALUES ('" 
                + WView.DocumentTitle + "', '" + WView.Source.ToString() + "', 1);").Step();
        }
        ...
        {
            objConn.Prepare("INSERT INTO favorites (name, url) VALUES ('" +
                AddFName.Text + "', '" + AddFURL.Text + "');").Step();
            ChangeFavButton(true);
        }

        private void CBUnFavorite_Click(object sender, RoutedEventArgs e)
        {
            objConn.Prepare("DELETE FROM favorites WHERE url='" +
                WView.Source.ToString() + "';").Step();
        ...
        {
            ShowMessage(loader.GetString("AppDeveloped") + " MediaExplorer", true);
        }
        ...
        public async void ShowMessage(string message, bool credits)
        {
            ...
            if(credits)
            {
                md.Commands.Add(new UICommand("GitHub", 
                    new UICommandInvokedHandler(CommandHandlers)) { Id = 0 });

                md.Commands.Add(new UICommand(loader.GetString("Close")) { Id = 1 });
            }
            else
            {
                md.Commands.Add(new UICommand(loader.GetString("Yes"),
        ...
        public void CommandHandlers(IUICommand commandLabel)
        {
            ...
            {
                case "GitHub":
                    WView.Source = new Uri("http://github.com/mediaexplorer74");
                    break;
                case "Yes":
                case "Sim":
        ...
        {
            CreateNewTab("Yandex", "http://ya.ru");
        }
